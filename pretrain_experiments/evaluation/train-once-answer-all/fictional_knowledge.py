...
def eval_fictional_knowledge(model :str, revision:str | None, task_file :str, responses_file :str = None, print_responses :bool = False):
    ...
    logger.info(f"Average probability of the target given the input: {np.mean(probabilities)}")
    ...
    for gen, target in zip(model_generations, targets):
        if print_responses:
            logger.info(f"Model Response: '{gen}' (Target: '{target}')")
        if target.strip().upper() in gen.upper():
            accuracies.append(1)
        else:
            accuracies.append(0)

    logger.info(f"Accuracy of generating the target at temperature 0: {np.mean(accuracies) * 100:.2f}%")
