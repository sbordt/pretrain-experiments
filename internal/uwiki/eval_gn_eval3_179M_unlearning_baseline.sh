...
#SBATCH --job-name=gn-eval3-179M-unlearning-baseline
...
# 179M unlearning-baseline at steps 102/104/106/108/110k. Mirrors
# eval_gn_eval3_1B_unlearning_baseline.sh scaled to 179M.
#
...
# to HF locally. Outputs go to evals/gn-eval3-sweep/179M/unlearning-baseline/.
#
# Local cache root is namespaced under checkpoints/179M-Exp-Unlearning/ to
# avoid colliding with the existing 1B unlearning-baseline-stage1-step*-hf
# dirs (those have hidden_size=2048, not 576).
...
HF_REPO="sbordt/OLMo-2-179M-Exp-Unlearning"
...
LOCAL_CACHE_ROOT=~/pretrain-experiments/checkpoints/179M-Exp-Unlearning
...
# checkpoints/179M-Exp-Unlearning/unlearning-baseline-stage1-step<N>-hf
...
# Verify a downloaded/converted HF checkpoint matches the 179M architecture
# expected by the 179M noise vectors. Aborts if hidden_size != 576 — guards
