...
# SimNPO unlearning sweep cell on OLMo-2-179M-Exp-Unlearning,
# starting from stage1-step100000-tokens210B.
#
# Required env vars:
#   LR               - learning rate (e.g. 1e-5, 5e-6)
#   BETA             - SimNPO inverse temperature β (e.g. 0.1, 0.5, 1.0, 2.5)
#
# Optional env vars:
#   GAMMA            - reward margin γ (default: 0.0)
#   RETAIN_W         - α on retain CE (default: 1.0; set 0 for forget-only)
...
#   RUN_TAG          - subdir under unlearning-simnpo/ (default: 179M-default)
...
: "${LR:?set LR}"
: "${BETA:?set BETA (e.g. 0.1, 0.5, 1.0, 2.5)}"
...
OUTPUT_DIR="$HOME/pretrain-experiments/unlearning-simnpo/${RUN_TAG}/lr${LR}-b${BETA}-g${GAMMA}-w${RETAIN_W}"
...
echo "============================================"
echo "  SimNPO cell: lr=$LR  β=$BETA  γ=$GAMMA  α_retain=$RETAIN_W"
...
echo "  dtype:        $DTYPE  grad_ckpt: $GRAD_CKPT"
...
echo ""
echo "============================================"
echo "  SimNPO cell DONE: lr=$LR β=$BETA γ=$GAMMA α=$RETAIN_W"
