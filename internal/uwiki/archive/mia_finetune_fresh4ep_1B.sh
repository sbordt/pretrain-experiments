...
#SBATCH --job-name=mia-ft-fresh4ep-1B
...
# Clean continuation of the 10-epoch MIA finetune (1B, run rd46npwd): 4 true
# epochs over a fresh canary-free 1.007B-token stage1 slice (steps
# 100480-100960), starting from step4800 of the 10-ep run (exposed as
# step0-unsharded). Unsharded checkpoints every epoch (steps 480/960/1440/1920).
...
START=~/pretrain-experiments/checkpoints/1B-DeepIgnorance-ft10ep/step0-unsharded
[ -s "$START/model.pt" ] || { echo "ERROR: $START/model.pt missing (broken symlink?)" >&2; exit 1; }

# Train 4 epochs.
python -m pretrain_experiments config/mia-finetune-fresh4ep-1B.yaml
