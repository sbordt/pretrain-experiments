...
case "$LABEL" in
  baseline)
    STEPS=(100000)
    # idspace-fix rerun: write parallel to the deep-ignorance idspace-fix results
    # (evals/mia-vllm-idspace-fix/baseline/step-100000/...) so the stale pre-fix
    # gn-eval3-sweep-fresh baseline results are left untouched.
    EVAL_ROOT=~/pretrain-experiments/evals/mia-vllm-idspace-fix
    ;;
  1e-5)
  ...
  unlearning-baseline)
    ...
    ;;
  unlearning-baseline-110k)
    # idspace-fix rerun of the unlearning baseline (continued training on
    # remaining data) at step 110000, loaded straight from HF @
    # stage1-step110000-tokens231B. Writes parallel to the other idspace-fix
    # results: evals/mia-vllm-idspace-fix/unlearning-baseline/step-110000/...
    STEPS=(110000)
    EVAL_ROOT=~/pretrain-experiments/evals/mia-vllm-idspace-fix
    ;;
  *)
...
resolve_model () {
  ...
  case "$LABEL" in
    ...
    unlearning-baseline-110k)
      MODEL_DIR="sbordt/OLMo-2-179M-Exp-Unlearning"
      MODEL_REVISION="stage1-step110000-tokens231B"
      ;;
    1e-5|1e-6|1e-7)
...
for STEP in "${STEPS[@]}"; do
  resolve_model "$STEP"
  # HF-loaded labels have no local model.safetensors to stat/arch-check.
  case "$LABEL" in
    baseline|unlearning-baseline-110k) ;;
    *)
