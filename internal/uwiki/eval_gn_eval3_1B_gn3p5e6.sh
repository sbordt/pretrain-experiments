...
#SBATCH --job-name=gn-eval3-1B-gn3p5e6
...
# 1B gradient-noise unlearning variant (sigma=3.5e-6, run 9050f9m3) at steps
# 102/104/106/108/110k. Matches the 179M/546M sweep convention for the
# noise-level labels (GW + MIA only, no FK/VM).
#
# Local checkpoints (already converted to HF) live under
# unlearning-gradient-noise/OLMo-2-1B-Exp-gradient-noise-9050f9m3/step<N>-hf.
# Outputs go to evals/gn-eval3-sweep/1B/3.5e-6/.
...
LABEL="3.5e-6"
RUN_DIR=~/pretrain-experiments/unlearning-gradient-noise/OLMo-2-1B-Exp-gradient-noise-9050f9m3
