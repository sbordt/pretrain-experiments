...
#SBATCH --job-name=gn-eval3-1B-deep-ignorance
...
# 1B deep-ignorance ground-truth at steps 100/102/104/106/108/110k. Mirrors
# eval_gn_eval3_1B_unlearning_baseline.sh but pulls from
# sbordt/OLMo-2-1B-Unlearning (no Exp-) and writes to a 1B-namespaced local
# cache to avoid colliding with the existing 179M deep-ignorance dirs at
# checkpoints/deep-ignorance-*-hf (those are hidden_size=576, not 2048).
#
# All steps are fetched as HF revisions (both 100k and 110k exist as
# `stage1-step100000-tokens210B` / `stage1-step110000-tokens231B`), so no
# unsharded->HF conversion is needed.
...
HF_REPO="sbordt/OLMo-2-1B-Unlearning"
...
LOCAL_CACHE_ROOT=~/pretrain-experiments/checkpoints/1B-Unlearning
...
# checkpoints/1B-Unlearning/deep-ignorance-stage1-step<N>-hf. The 1B-Unlearning
# prefix avoids collision with the existing 179M-class
# checkpoints/deep-ignorance-*-hf dirs.
...
# Verify a downloaded HF checkpoint matches the 1B architecture expected by
# the 1B noise vectors. Aborts if hidden_size != 2048 — guards against the
# 2026-05 incident where mislabelled-size weights silently lived behind
# stage1-step branches.
assert_1b_arch () {
  local hf_dir=$1
  python - "$hf_dir" <<'PY'
import json, sys
cfg = json.load(open(f"{sys.argv[1]}/config.json"))
exp = {"hidden_size": 2048, "intermediate_size": 8192,
       "num_hidden_layers": 16, "num_attention_heads": 16}
got = {k: cfg.get(k) for k in exp}
if got != exp:
    print(f"ERROR: HF config in {sys.argv[1]} does not match 1B arch.\n"
