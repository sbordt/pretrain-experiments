...
#SBATCH --job-name=gn-eval3-546M-deep-ignorance
...
# 546M deep-ignorance ground-truth at steps 100/102/104/106/108/110k. Mirrors
# eval_gn_eval3_1B_deep_ignorance.sh but pulls from sbordt/OLMo-2-546M-Unlearning
# (no Exp-) and writes to a 546M-namespaced local cache to avoid colliding with
# the existing 179M-class deep-ignorance dirs at checkpoints/deep-ignorance-*-hf
# (those are hidden_size=576, not 1120).
#
...
HF_REPO="sbordt/OLMo-2-546M-Unlearning"
...
LOCAL_CACHE_ROOT=~/pretrain-experiments/checkpoints/546M-Unlearning
...
# checkpoints/546M-Unlearning/deep-ignorance-stage1-step<N>-hf. The 546M-Unlearning
...
# Verify a downloaded HF checkpoint matches the 546M architecture expected by
# the 546M noise vectors. Aborts if hidden_size != 1120.
assert_546m_arch () {
  local hf_dir=$1
  python - "$hf_dir" <<'PY'
import json, sys
