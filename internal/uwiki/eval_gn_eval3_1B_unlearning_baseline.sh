...
#SBATCH --job-name=gn-eval3-1B-unlearning-baseline
...
# 1B unlearning-baseline at steps 102/104/106/108/110k. Matches the 179M/546M
# sweep convention for the unlearning-baseline label (GW + MIA only, no FK/VM).
#
...
# to HF locally. Outputs go to evals/gn-eval3-sweep/1B/unlearning-baseline/.
...
HF_REPO="sbordt/OLMo-2-1B-Exp-Unlearning"
...
# checkpoints/unlearning-baseline-stage1-step<N>-hf (matches sweep convention).
...
# Verify a downloaded/converted HF checkpoint matches the 1B architecture
# expected by the 1B noise vectors. Aborts if hidden_size != 2048 — guards
# against the 2026-05 incident where stage1-step10{2,4,6,8}000 on
# sbordt/OLMo-2-1B-Exp-Unlearning silently contained 179M-class weights.
assert_1b_arch () {
  local hf_dir=$1
  python - "$hf_dir" <<'PY'
import json, sys
...
for STEP in "${STEPS[@]}"; do
  if [ "$STEP" = "110000" ]; then
    UNSHARDED=~/pretrain-experiments/checkpoints/1B-Exp-Unlearning/step110000-unsharded
    HF_DIR=~/pretrain-experiments/checkpoints/1B-Exp-Unlearning/step110000-hf
    ...
    HF_DIR=~/pretrain-experiments/checkpoints/unlearning-baseline-stage1-step${STEP}-hf
